...
class Robot:
    ...
    def is_behind_hq(self):
        return dist(self.hq_loc, self.enemy_hq_loc) < dist(self.location, self.enemy_hq_loc)
        

    def build_up(self):
        if self.is_behind_hq():
            self.move_towards(self.enemy_hq_loc)
            return
        if dist(self.location, self.hq_loc) < 100:
            if self.move_towards(self.enemy_hq_loc):
                return
...
class Builder(Robot):
    ...
    def __init__(self):
        ...
        self.purpose = None
#        for block in chain:
#            if block[1] == BUILDER_BUILT:
#                self.purpose = self.purpose_map[block[4]]
#        assert(self.purpose != None)
        self.purpose = "R"
        self.refineries = 0
        self.barracks = 0
