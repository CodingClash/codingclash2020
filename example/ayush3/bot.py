...
class Builder:
    ...
    def try_create(self):
        ...
        if len(self.spawned)<2:
            ...
            if len([1 for t in dxdy if sense_location((self.location[0] + t[0], self.location[1] + t[1])).type == RobotType.NONE])<2: return False
            if get_oil() < 1.5*cost:
                return False
            dlog(str(self.location)+" got here")
            for (dx, dy) in dxdy:
                if dx == 0 and dy == 0:
                    continue
                loc = (self.location[0] + dx, self.location[1] + dy)
                if not can_sense_location(loc):
                    continue
                if sense_location(loc).type == RobotType.NONE and self.distance_2(loc, self.my_hq)>8 and (([i.team for i in sense()].count(get_team()) >5 and self.distance_2(loc, self.my_hq)<self.distance_2(self.location, self.my_hq)) or ([i.team for i in sense()].count(get_team()) <= 5 and self.distance_2(loc, self.my_hq)<self.distance_2(self.location, self.opp_hq))):
                    self.spawned.append(robot)
                    create(robot, loc)
                    return True
    ...
    def try_move(self):
        ...
        if self.should_move:
            if (get_round_num()!=0 and [self.secret, 100, 100, 0, 0] in get_blockchain(get_round_num()-1)) or len([1 for t in sense() if t.team!=get_team()])>0:
            ...
            for dx, dy in options:
                loc = (self.location[0] + dx, self.location[1] + dy)
                if not can_sense_location(loc) or (self.opp_hq==self.my_hq and self.distance_2(loc, self.opp_hq) <4):
                    continue
                if self.opp_hq!=self.my_hq and [(i.type, i.team) for i in sense()].count((RobotType.TANK, get_team())) < 6:
                    return
                if sense_location(loc).type == RobotType.NONE:
                    move(loc)
                    return
...
class Gunner:
    ...
    def try_move(self):
        ...
        if [(i.team, i.type) for i in sense()].count((get_team(), RobotType.GUNNER)) < 1: return
        for dx, dy in options:
            loc = (self.location[0] + dx, self.location[1] + dy)
            if sense_location(loc).type == RobotType.NONE:
                move(loc)
...
class Tank:
    ...
    def try_move(self):
        ...
        if [(i.type, i.team) for i in sense()].count((RobotType.TANK, get_team())) < 3: return
        for dx, dy in options:
            loc = (self.location[0] + dx, self.location[1] + dy)
            if sense_location(loc).type == RobotType.NONE:
                move(loc)
